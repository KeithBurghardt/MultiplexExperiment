...
public class Agent {
	...
	double util_before_turn;
	boolean IsConfederate = false;
	int IgnoreNode = 0;
	/*
	 * GET methods (for viz)
	 */
	...
	public double[] bestAdd(AgentsSimulation as, int searchSize){	
		...
		// only search N-1 nodes, 
		// but if we search through "N" nodes, we search through each one once
	 ...
	 public boolean addRewire(AgentsSimulation as, Agent other, int addLayer,int searchSize){
	  ...
	  while(dropCount < max)
	  {
	   ...
	   { //random agent
	    ...
	    if(countDROPPLAYER < searchSize && other.index!=i && as.coplayerMatrix[dropLayer][other.index][i])// && other.can_play && as.agentList[i].can_play)
	    {
	  ...
	  if(otherAddUtil > otherCurrentUtil||otherRewireUtil > otherCurrentUtil){ //if increases utility or by chance
	   if(otherAddUtil > otherRewireUtil)
	   {
